...
public interface PostRepository extends JpaRepository<Post, UUID> {
    ...
    @Query("select post from Post as post left join post.postTags as tag where post.deleted = false and upper(post.openType) like upper(:#{#findPublicPostOnly ? 'PUBLIC' : '%'}) and post.blogId = :blogId and (:tag is null or tag.tag.name = :tag) order by post.createdAt desc, post.id desc")
